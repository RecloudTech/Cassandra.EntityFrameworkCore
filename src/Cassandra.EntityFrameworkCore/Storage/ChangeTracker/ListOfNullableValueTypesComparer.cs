...
namespace Cassandra.EntityFrameworkCore.Storage.ChangeTracker;

/// <summary>
///     A <see cref="ValueComparer" /> for lists of primitive items. The list can be typed as <see cref="IEnumerable{T}" />
///     ,
///     but can only be used with instances that implement <see cref="IList{T}" />.
/// </summary>
/// <remarks>
///     <para>
///         This comparer should be used for nullable value types. Use
///         <see cref="ListOfNullableValueTypesComparer{TConcreteCollection,TElement}" /> for reference
...
public sealed class
    ListOfNullableValueTypesComparer<TConcreteCollection, TElement> : ValueComparer<IEnumerable<TElement?>>
    where TElement : struct
{
    ...
    private static readonly bool IsReadOnly = IsArray
                                              || (typeof(TConcreteCollection).IsGenericType
                                                  && typeof(TConcreteCollection).GetGenericTypeDefinition() ==
                                                  typeof(ReadOnlyCollection<>));

    private static readonly MethodInfo CompareMethod =
        typeof(ListOfNullableValueTypesComparer<TConcreteCollection, TElement>).GetMethod(
    ...
    private static readonly MethodInfo GetHashCodeMethod =
        typeof(ListOfNullableValueTypesComparer<TConcreteCollection, TElement>).GetMethod(
    ...
    private static Expression<Func<IEnumerable<TElement?>?, IEnumerable<TElement?>?, bool>> CompareLambda(
    ...
    private static Expression<Func<IEnumerable<TElement?>, IEnumerable<TElement?>>> SnapshotLambda(
        ValueComparer elementComparer)
    {
    ...
    private static bool Compare(IEnumerable<TElement?>? a, IEnumerable<TElement?>? b,
    ...
    private static int GetHashCode(IEnumerable<TElement?> source, ValueComparer<TElement?> elementComparer)
    {
        ...
        foreach (var el in source) hash.Add(el == null ? 0 : elementComparer.GetHashCode(el));
    ...
    private static IList<TElement?> Snapshot(IEnumerable<TElement?> source, ValueComparer<TElement?> elementComparer)
    {
        ...
        {
            var snapshot = IsReadOnly
                ? new List<TElement?>()
                : (IList<TElement?>)Activator.CreateInstance<TConcreteCollection>()!;
            foreach (var e in sourceList) snapshot.Add(e == null ? null : elementComparer.Snapshot(e));
