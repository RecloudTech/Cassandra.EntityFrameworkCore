    ...
    where TElement : struct
{
    ...
    private static bool Compare(TCollection? a, TCollection? b, ValueComparer<TElement> elementComparer)
    {
        ...
        if (b is not IReadOnlyDictionary<string, TElement?> bDict || aDict.Count != bDict.Count) return false;

        if (ReferenceEquals(aDict, bDict)) return true;

        foreach (var (key, element) in aDict)
        {
            if (!bDict.TryGetValue(key, out var bValue)) return false;

            if (element is null)
            {
                if (bValue is null) continue;

                return false;
            }

            if (bValue is null || !elementComparer.Equals(element, bValue)) return false;
        }
